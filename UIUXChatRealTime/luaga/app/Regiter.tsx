...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    if (!name || !email || !password) return Alert.alert('Lỗi', 'Vui lòng điền đủ thông tin!');

    setIsLoading(true);
    try {
      await axios.post(
        'https://ungentlemanlike-aspen-electronically.ngrok-free.dev/api/OAuth/register', 
        { ten: name, email, matKhau: password }
      );
      Alert.alert('Thành công', 'Đăng ký thành công! Vui lòng đăng nhập.', [{ text: 'OK', onPress: () => router.replace('/Login') }]);
    } catch (error) {
      const e = error as AxiosError<string>;
      Alert.alert('Đăng ký thất bại', e.response?.data || 'Lỗi server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}><Ionicons name="arrow-back" size={28} color="#D4AF37" /></TouchableOpacity>
